...
class CartPage extends React.Component {
                ...
                this.props.cart.map((item, index) => {
                    if (index === this.state.indexEdit) {
                        return ( <
                            ...
                            onChange = {
                                (e) => this.onChangeQty(e, item.stock) }
                            /> <
                            ...
                            /td> <
                            td > IDR {
                                (item.price * item.qty).toLocaleString() }, 00 < /td> <
                            td >
                            <
                            Button variant = "success"
                            className = "mr-2"
                            onClick = {
                                () => this.onSave(index) } > Save < /Button> <
                            Button variant = "danger"
                            onClick = {
                                () => this.setState({ indexEdit: null }) } > Cancel < /Button> <
                            /td> <
                            /tr>
                        )
                    }
                    return ( <
                        ...
                        Button variant = "danger"
                        onClick = {
                            () => this.onDelete(index) } > Delete < /Button> <
                        /td> <
                        /tr>
                    )
                })
    ...
    render() {
        ...
        return ( <
            ...
            <
            h1 style = {
                { marginTop: '10vh' } } > Cart Page < /h1> <
            Table style = { styles.table }
            striped bordered hover variant = "dark" > { this.showTableHead() } { this.showTableBody() } <
            /Table> <
            /div>
        )
    }
